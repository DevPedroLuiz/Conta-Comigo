...
const getAccountIcon = (type: string) => {
  switch (type) {
    case 'CHECKING_ACCOUNT': return <Landmark className="h-5 w-5 text-zinc-400" />;
...
export function AccountCard({ account, onDelete }: AccountCardProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-lg border border-zinc-800 bg-[#0c0c0e] hover:border-zinc-700 transition-colors">
      <div className="flex items-center gap-4 mb-4 sm:mb-0">
        <div className="flex items-center justify-center h-10 w-10 rounded-full bg-zinc-800/50">
          {getAccountIcon(account.type)}
        </div>
        <div>
          <h3 className="font-semibold text-lg">{account.name}</h3>
          <p className="text-sm text-zinc-400">{getAccountTypeName(account.type)}</p>
        </div>
      </div>
      
      <div className="flex items-center justify-between w-full sm:w-auto gap-6">
        <div className="text-right">
          <p className={`font-semibold text-lg ${account.current_balance >= 0 ? 'text-emerald-500' : 'text-destructive'}`}>
            {formattedBalance}
          </p>
        </div>
